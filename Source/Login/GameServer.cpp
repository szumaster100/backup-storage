...
CGameServer::~CGameServer()
{
	...
	wsprintf(Txt100, "(ERROR) GameServer [%s] connection was lost!", ServerName);
	PutLogList(Txt100, WARN_MSG);
	ConnectedHGServers--;
}
//==============================================================================
void CGameServer::SendMsg(char * Data, DWORD dwSize, char cKey, BOOL log)
{
	if(SocketIndex[0] < 0) return;
	if(GameServerSocket[SocketIndex[0]] != NULL) GameServerSocket[SocketIndex[0]]->iSendMsg(Data, dwSize, cKey, log);
}
//=============================================================================
BOOL CGameServer::bStockMsgToGameServer(char *Data, DWORD dwSize)
{
	char * cp;
